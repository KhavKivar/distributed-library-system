...
func uploadFileRandomDistribuido(f string, name string, etx string) {
	conexiones := [3]string{address1, address2, address3}
	rand.Seed(time.Now().UnixNano())
	numeroRandom := rand.Intn(3)
	elegido := conexiones[numeroRandom]
	...
	ctx, cancel := context.WithTimeout(context.Background(), 4*time.Second)
	defer cancel()
	uploadFileDistribuido(ctx, c, f, name, etx)
}

func uploadFileDistribuido(ctx context.Context, c pb.EstructuraCentralizadaClient, f string, name string, etx string) (stats pb.UploadStatus, err error) {
	fileToBeChunked := f
	file, err := os.Open(fileToBeChunked)
	if err != nil {
		fmt.Println(err)
	...
	for i := uint64(0); i < totalPartsNum; i++ {
		...
		partBuffer := make([]byte, partSize)
		file.Read(partBuffer)
		re, _ := c.SubirDistribuida(ctx, &pb.Chunk{Contenido: partBuffer, TotalChunks: int32(totalPartsNum), NumeroChunk: int32(i), Nombre: name, Ext: etx})
		mensaje := re.GetMensaje()
...
func rutinaUploadFilesDistribuido(folder string, dontInclude string) {
	var files []string
	root := folder
...
func main() {
	...
	for true {
		...
		fmt.Print("Ingrese el servidor a iniciar centralizado(1) o distribuido(2)\n")
